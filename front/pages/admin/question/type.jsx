...
const Type = () => {
  ...
  useEffect(() => {
    if (st_questionTypeCreateDone) {
      message.success("문의 유형이 생성되었습니다.");
  ...
  useEffect(() => {
    if (st_questionTypeUpdateDone) {
      message.success("문의 유형이 수정되었습니다.");
  ...
  useEffect(() => {
    if (st_questionTypeDeleteDone) {
      message.success("문의 유형이 삭제되었습니다.");
  ...
  return (
    <AdminLayout>
      ...
      >
        <Wrapper dr={`row`} ju={`space-between`} padding={`10px`}>
          <Text>문의 유형 :</Text>

          <Input style={{ width: `85%` }} {...inputValue} />
        </Wrapper>
      </Modal>
