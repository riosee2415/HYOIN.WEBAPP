...
import { NEWS_LIST_REQUEST } from "../../../reducers/news";
...
const Index = () => {
  ...
  const { newsList, newsLen, lastPage, st_newsListError } = useSelector(
    (state) => state.news
  );
  ...
  useEffect(() => {
    dispatch({
      ...
      data: {
        page: currentPage,
        searchTitle: searchNews.value,
      },
    });
  }, [currentPage, searchNews.value]);
  ////// TOGGLE //////
  ...
  const detailHandler = useCallback((data) => {
    moveLinkHandler(`/garden/news/${data.id}`);
    ...
    <>
      ...
      <ClientLayout>
        <WholeWrapper>
          <SubBanner />
          <RsWrapper>
            <CommonTitle margin={`85px 0 17px`}>새소식</CommonTitle>

            <Text fontSize={width < 700 ? `14px` : `16px`}>
              효원요양원의 다양한 소식을 전해드리겠습니다.
            </Text>
